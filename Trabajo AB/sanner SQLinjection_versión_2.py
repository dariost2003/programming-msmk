...
def get_links(TARGET_URL):
	...
	referencias = {enlace.get("href") for enlace in soup.find_all("a") if enlace.get("href")}
	return referencias

def scan_sql_injection(TARGET_URL):
	hallazgo = 'N' # Payloads diseñados para extraer datos visibles
	payloads = {							
			...
			if len(tablas_encontradas) == 0:
				...
				if table:
					...
					if nombre_prueba == "Prueba clásica de bypass":
						...
						print(Fore.YELLOW + f"[!] Medio: Resultado positivo. Admite SQL injectión.\n")
						print("	Evidencia:")
						...
						for nombre_tabla in tablas_credenciales:
							if nombre_tabla in cells_text:
								...
								print(f"	Payload: {payload}\n")
								print(Fore.RED + f"[!] CRÍTICO: Vulnerabilidad confirmada en '{nombre_prueba}'")
				...
				for encontrada in tablas_encontradas:
					...
					if table:
						...
						if len(cells_text) > 0:							
							...
							print(Fore.RED +  f"[!] CRÍTICO: Fuga de datos detectada en '{nombre_prueba}'")
	...
	if hallazgo == 'N':
		print(Fore.GREEN + 'Sin hallazgo')

if __name__ == "__main__":
	init(autoreset=True)
	if len(sys.argv) != 2:
		print('Uso: python scanner.py <URL:PUERTO> ')		
		sys.exit(1)
	lista_links = get_links(sys.argv[1])
	print(lista_links)
	for link in lista_links: 
    		scan_sql_injection(sys.argv[1]+link)
